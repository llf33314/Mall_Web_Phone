<template>
  <div>
    <div class="groupingModule">
        ...
        <table>
          <tr>
            <td class="grouping-leftNav-l">
              <div v-for="(pic,index) in picJson.imgID" :class="{'index':index==select}" @click="tapChange(index)"><a>{{pic.groupName}}</a></div>
            </td>
            <td class="grouping-leftNav-r" v-if="list.productList">
              <div class="grouping-leftNav-commodity" v-for="pic in list.productList.subList">
                <div class="grouping-leftNav-commodity-pic"><img :src="pic.image_url"></div>
                <div>
                  <p class="grouping-leftNav-r-title">{{pic.pro_name}}</p>
                  <span class="grouping-leftNav-r-price">￥{{pic.price}}</span>
                  <a class="grouping-leftNav-r-buy" href="javascript:void(0)"><span></span></a>
                </div>
              </div>
            </td>
          </tr>
      ...
      <!-- 顶部菜单 -->
      <div class="grouping-topNav" v-if="data.nav==1">
      ...
      <div v-if="data.nav==1">
        ...
        <div v-if="data.page==0" class="goods-card goods-card02">
          <ul class="sc-goods-list clearfix"  v-if="list.productList">
            <li v-for="(pic,index) in list.productList.subList">
              <a :href="pic.url" class="link clearfix" :class="{'simpleness':data.css==1}">
                <div class="photo-block">
                  <div class="photo-block img_class bg-con h292" style="display: block;" v-lazy:background-image="pic.image_url"></div>
                </div>
          ...
          <ul class="sc-goods-list clearfix" v-if="list.productList">
            <li v-for="pic in list.productList.subList">
              <a :href="pic.url" class="link clearfix" :class="{1:'simpleness'}[data.css]">
                ...
                <div class="photo-block img_class bg-con" v-lazy:background-image="pic.image_url"></div>
                <div v-if="data.css == 0">
                  <div class="clearfix  info-price" v-if="data.name || data.price">
                    <p class="goods-title"><span class="goods-label" v-if="data.label && pic.pro_label">{{pic.pro_label}}</span><span>{{pic.title}}</span></p>
